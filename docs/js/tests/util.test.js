...
describe('getStartTime', () => {
    test('should return creation time if createCard event is for the correct list', () => {
        ...
        expect(getStartTime([createCard], slaCondition("list1"))).toEqual(novemberDateUnix);
        expect(getStartTime([updateCard, createCard], slaCondition("list1"))).toEqual(novemberDateUnix);
        expect(getStartTime([createCard, updateCard], slaCondition("list1"))).toEqual(novemberDateUnix);
    });
    test('should not return time if createCard event is for incorrect list', () => {
        expect(getStartTime([createCardAction("list2", novemberDate)], slaCondition("list1"))).toEqual(undefined);
    });
    test('should return correct time if updateCard event is for correct list', () => {
        ...
        expect(getStartTime([updateCardNovember, updateCardDecember], slaCondition("list1"))).toEqual(novemberDateUnix);
        expect(getStartTime([updateCardDecember, updateCardNovember], slaCondition("list1"))).toEqual(novemberDateUnix);
    });
});
describe('getEndTime', () => {
    test('should return correct time after startCondition fulfilment time', () => {
        ...
        expect(getEndTime([updateCardOctober, updateCardNovember, updateCardDecember], slaCondition("list2"), octoberDateUnix + 20000)).toEqual(decemberDateUnix);
    });
    test('should return correct time after startCondition fulfilment time', () => {
        const updateCardOctober = updateCardAction("list2", octoberDate);
        const updateCardNovember = updateCardAction("list2", novemberDate);
        const updateCardDecember = updateCardAction("list2", decemberDate);
        expect(getEndTime([updateCardOctober, updateCardNovember, updateCardDecember], slaCondition("list2"), octoberDateUnix - 10000)).toEqual(octoberDateUnix);
    });
    describe('getColor', () => {
